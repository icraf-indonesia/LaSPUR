...
recommendation_adjacent_server <- function(id, output_dir) {
  moduleServer(id, function(input, output, session) {
      ...
      leaflet::leaflet(map_sf) %>%
        ...
        leaflet::addPolygons(
          ...
          label = ~paste0(
            ...
            "<strong>Integrasi aktual:</strong> ", actual_integration, "<br>",
            "<strong>Integrasi baru:</strong> ", recom_integration
    ...
    output$dl_gpkg <- downloadHandler(
      filename = function() "idx_padan_recommendation.gpkg",
      content = function(file) {
        req(rv$final_result)
    ...
    output$dl_xlsx <- downloadHandler(
      filename = function() "idx_padan_recommendation.xlsx",
      content = function(file) {
        req(rv$final_result)
